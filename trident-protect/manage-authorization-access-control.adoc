...
summary: 'Trident Protect verwendet das Kubernetes-Modell der rollenbasierten Zugriffskontrolle (RBAC). Mit den RBAC-Funktionen von Trident Protect können Sie den Zugriff auf Ressourcen präziser steuern.' 
---
...
====
Weitere Informationen darüber, wie RBAC den Zugriff auf Ressourcen und Namensräume regelt, finden Sie in der https://kubernetes.io/docs/reference/access-authn-authz/rbac/["Kubernetes RBAC-Dokumentation"^] .

Weitere Informationen zu Servicekonten finden Sie unter https://kubernetes.io/docs/tasks/configure-pod-container/configure-service-account/["Dokumentation zum Kubernetes-Dienstkonto"^] .
...
.Schritte
. Erstellen und Anwenden einer YAML-Datei mit einer Kombination aus AppVault und Geheimnis, die einem Benutzer Zugriff auf einen AppVault gewährt.  Beispielsweise gewährt die folgende CR dem Benutzer Zugriff auf einen AppVault. `eng-user` :
+
[source, yaml]
----
apiVersion: v1
