...
--
====
. Optional kann auf dem Quellcluster ein Snapshot der Quellanwendung erstellt werden.  Dieser Snapshot dient als Grundlage für die Anwendung auf dem Zielcluster.  Wenn Sie diesen Schritt überspringen, müssen Sie warten, bis der nächste geplante Snapshot erstellt wird, damit Sie über einen aktuellen Snapshot verfügen.
+
[NOTE]
====
...
[role="tabbed-block"]
====
.Erstellen Sie einen Schnappschuss mit einer CR
--
...
*** *spec.storageClassName*: (_Erforderlich_) Wählen Sie den Namen einer gültigen Speicherklasse im Cluster.  Die Speicherklasse muss mit einer ONTAP Speicher-VM verknüpft sein, die mit der Quellumgebung per Peering verbunden ist.
*** *spec.recurrenceRule*: Definiert ein Startdatum in UTC-Zeit und ein Wiederholungsintervall.
+
Beispiel-YAML:
...
--
.. Erstellen und wenden Sie das AppMirrorRelationship-Objekt an und ersetzen Sie die Werte in Klammern durch Informationen aus Ihrer Umgebung. Beispiel:
+
[source, console]
----
