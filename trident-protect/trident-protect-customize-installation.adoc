...
---
= Trident Schutzinstallation anpassen
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== Legen Sie die Ressourcengrenzen des Trident Schutzcontainers fest.
...
Sie können eine Konfigurationsdatei verwenden, um die OpenShift-Sicherheitskontextbeschränkungen (SCCs) für Trident Protect-Container zu ändern, nachdem Sie Trident Protect installiert haben.  Diese Einschränkungen definieren Sicherheitsbeschränkungen für Pods in einem Red Hat OpenShift-Cluster.
...
| autoSupport.enabled | boolescher Wert | Aktiviert oder deaktiviert tägliche Uploads des Trident Protect AutoSupport Pakets. Wenn eingestellt auf `false` Tägliche Uploads sind deaktiviert, Support-Bundles können aber weiterhin manuell generiert werden.  (Standard: `true` ) 
...
== Trident Schutz-Pods auf bestimmte Knoten beschränken
